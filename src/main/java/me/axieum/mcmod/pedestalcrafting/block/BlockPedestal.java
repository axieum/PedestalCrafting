...
public class BlockPedestal extends BlockTileBase<TilePedestal>
{
    ...
    {
        return state.getValue(VARIANT).getName().equals("glowstone") ? 14 : 0;
    }
    ...
    public static void onColorHandle(ColorHandlerEvent.Block event)
    {
        event.getBlockColors().registerBlockColorHandler(
                ...
                {
                    if (state != ModBlocks.PEDESTAL.getDefaultState().withProperty(
                            VARIANT,
                            Variant.GRASS
                    ))
                        return -1;
    ...
    @Override
    public Class<TilePedestal> getTileEntityClass()
    {
    ...
    @Nullable
    @Override
    public TilePedestal createTileEntity(World world, IBlockState state)
    {
