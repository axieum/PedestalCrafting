...
public class RenderPedestalCore extends TileEntitySpecialRenderer<TilePedestalCore>
{
    @Override
    public void render(TilePedestalCore tile, double x, double y, double z, float partialTick, int destroyStage,
                       float alpha)
    {
        IBlockState state = tile.getWorld().getBlockState(tile.getPos());

        if (state == null || !(state.getBlock() instanceof BlockPedestalCore))
            return;
